...
sample_time <- sort(unique(c(mean_values$Time, seq(0, 90*24, 0.1))))  
...
predictions <- solution[,c('time', 'C_Lungs', 'C_Liver', 'C_Spleen', 'C_Kidneys', 'C_Blood')]
...
create.plots <- function(observations, predictions, compartment, y_max =NULL){  
  
  concentration_names <- c('C_Kidneys', 'C_Spleen', 'C_Lungs','C_Liver', 'C_Blood'  )
  selected_compartment = concentration_names[which(plot_compartment == compartment)]
  print(selected_compartment)
  if(is.null(y_max)){
    y_max = max(c(max(observations[compartment]),  max(predictions[selected_compartment])))
  }
  test_plot = ggplot()+
    geom_line(data=predictions, aes(x= time/24, y= !!as.name(selected_compartment), color = "Predictions"), size=1.5,alpha = 0.7) +
    geom_point(data=observations, aes(x=Time/24, y= !!as.name(compartment), color='Observations'), size=4)+
    #scale_y_continuous(trans='log10')+
    #if (!is.null(y_max)) ylim(0, y_max) else NULL +  # Use ifelse to conditionally set ylim
    ylim(0, 1.1*y_max)+
...
p5 <-  create.plots(mean_values, predictions, plot_compartment[5], y_max=0.1)
...
colnames(predictions) <- c("Time", "Lungs", "Liver", "Spleen", "Kidneys", "Blood")

# Exclude brain data
mean_values <- mean_values[,-which(colnames(mean_values) == "Brain")]
...
Study_name <- c("Disdier et al. (2015)")
# loop over tissues
for (j in 2:dim(mean_values)[2]) {
  
  added_data <- cbind.data.frame(Study_name , "21.5 nm", 75, 25, dose, 1,   colnames(mean_values)[j] ,mean_values[,j], predictions[which(predictions$Time %in% mean_values$Time), 
