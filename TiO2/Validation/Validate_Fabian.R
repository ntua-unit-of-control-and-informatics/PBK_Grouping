...
sample_time <- sort(unique(c(mean_values$Time, seq(0, 28*24, 0.1))))  
...
predictions <- solution[,c('time', 'C_Lungs', 'C_Liver', 'C_Spleen', 'C_Kidneys')]
...
create.plots <- function(observations, predictions, compartment){  
  ...
  concentration_names <- c('C_Kidneys', 'C_Spleen', 'C_Lungs','C_Liver'  )
...
plot_compartment <- c("Kidneys", "Spleen", "Lungs",  "Liver")
...
Study_name <- c("Fabian et al. (2008)")
# loop over tissues
for (j in 2:dim(mean_values)[2]) {
  
  added_data <- cbind.data.frame(Study_name, "25 nm", 70, 30, dose, 1, colnames(mean_values)[j] ,mean_values[,j], predictions[which(predictions$Time %in% mean_values$Time), 
