...
create.params  <- function(user_input){
  with( as.list(user_input),{
    ...
    VKC = 0.0073 #fraction vol. of kidney (L/kg BW); Brown 1997
    ...
    Vven_plasma = VPlas*11.3/(11.3+5.4) 	#volume of venous plasma (L) from doi:10.1007/bf02353860
    Vart_plasma = VPlas* 5.4/(11.3+5.4) 	#volume of arterial plasma (L)from doi:10.1007/bf02353860
    ...
    return(list( "QC" = QC, "QK" = QK, "QL_hepatic_artery" = QL_hepatic_artery, "Qrest" = Qrest, 
                 ...
                 "Pliver" = Pliver,  "Prest" = Prest, 
                 "Pintestine" = Pintestine, "Pgonads" = Pgonads,
                 "Pspleen" = Pspleen, "Pheart" = Pheart,
                 "Plung" = Plung, "Pbrain" = Pbrain, "Pstomach" =Pstomach, 
...
ode.func <- function(time, inits, params, custom.func){
  with(as.list(c(inits,params)),{
    ...
    CKidneyblood = AKidneyblood/VKb	#concentration in kidney blodd (mg/L) 
    ...
    Cven_free = Aven_free/Vven_plasma		#concentration in plasma (mg)
    Cven = Cven_free/Free	#concentration of total PFOA in plasma (mg/L)
...
#6. User input 
#=============
# Parameters for reproducing Figure 2 of Worley et al. 2015
sex <- "M" # rat sex, values: M/F
BW <- 0.3# rat body weight in kg
admin.type <-  "iv" # administration type values: iv/oral
admin.dose <- 1 * BW # administered dose in mg
admin.time <- 0.01 # time when doses are administered, in hours
...
sample_time <- seq(0,600,0.1)  #hours
